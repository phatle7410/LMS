        ...
        <table className='table-fixed md:table-auto w-full overflow-hidden pb-4'>
          <thead className='text-gray-900 border-b border-gray-500/20 text-sm text-left'>
            <tr>
              ...
              <th className='px-4 py-3 font-semibold HIDDEN SM:TABLE-CELL'>Thời Gian</th>
            </tr>
          </thead>
          <tbody className='text-sm text-gray-500'>
            {enrolledStudents.map((item, index) =>(
              <tr key={index} className='border-b border-gray-500/20 hover:bg-gray-100/50'>
                ...
                <td className='px-4 py-3 hidden sm:table-cell'>{new Date(item.joinDate).toLocaleDateString()}</td>
                </tr>
            ))}
          </tbody>
        </table>
