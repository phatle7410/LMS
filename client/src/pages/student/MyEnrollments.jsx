...
const MyEnrollments = () => {
  ...
  return (
    ...
    <div className='md:px-36 px-8 pt-10'>
      <h1 className='text-2xl font-semibold'>Các Bài Học Của Tôi</h1>
      <table className='md:table-auto table-fixed w-full overflow-hideen border mt-10'>
            ...
            <tr key={index} className='border-b border-gray-500/20'>
                <td className='md:px-4 pl-2 md:pl-4 py-3 flex items-center space-x-3'>
                  ...
                  <div className='flex-1'>
                    ...
                    <Line strokeWidth={2} percent={progressArray[index] ? (progressArray[index].lectureCompleted * 100) / progressArray[index].totalLectures : 0} className='gb-gray-300 rounded-full'/>
                  </div>
                </td>
                ...
                <td className='px-4 py-3 max-sm:text-right'>
                  <button className='px-3 sm:px-5 py-1.5 sm:py-2 bg-blue-600 max-sm:Text-xs text-white' onClick={()=> navigate('/player/' + course._id)}>
                    {progressArray[index] && progressArray[index].lectureCompleted / progressArray[index].totalLectures === 1 ? 'Hoàn Thành' : 'Chưa Hoàn Thành'}
                    </button>
                </td>
            </tr>
          ))}
